...
typedef struct {
	int m_Width;
	COLORREF m_Color;
}RndHole2FlagAttributes;

class CCadHoleRnd2Flat:public CCadObject
{
	RndHole2FlagAttributes m_Attrb;
public:
	CCadHoleRnd2Flat();
	virtual ~CCadHoleRnd2Flat();
	static void LoadSettings(RndHole2FlagAttributes *pAttrb);
	static void SaveSettings(RndHole2FlagAttributes *pAttrb);
	...
	void CopyAttributes(RndHole2FlagAttributes *pAttrb);
